...
def admin_dashboard():
    ...
    return render_template_string('''
        ...
        <script>
            ...
            document.getElementById('addSerialForm').addEventListener('submit', async function(e) {
                e.preventDefault();
            ...
            async function updateStatus(serialNumber) {
                try {
                    const statusSelect = document.getElementById(`status_${serialNumber}`);
                    const newStatus = statusSelect.value === 'true';
                    
                    console.log('Updating status for:', serialNumber, 'New status:', newStatus);
