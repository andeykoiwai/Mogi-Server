...
def upload_audio():
    if request.method == 'POST':
        try:
            ...
            # Cek apakah user meminta untuk membaca pesan
            if any(phrase in esp_user.lower() for phrase in ['baca pesan masuk', 'bacakan pesan', 'bacain pesan dong','baca pesan']):
                # Ambil pesan yang belum dibaca dari database
                conn = sqlite3.connect('mogi_messages.db')
                c = conn.cursor()
                c.execute('''SELECT id, from_serial, content, timestamp 
                           FROM messages 
                           WHERE to_serial = ? AND is_read = 0 
                           ORDER BY timestamp DESC''', (serial_number,))
                unread_messages = c.fetchall()
                
                if unread_messages:
                    # Format pesan untuk dibacakan
                    server_user = "Anda memiliki " + str(len(unread_messages)) + " pesan baru:\n\n"
                    for msg in unread_messages:
                        server_user += f"Pesan dari {msg[1]} pada {msg[3]}:\n{msg[2]}\n\n"
                    ...
                    server_user = "Tidak ada pesan baru yang belum dibaca."
                
                conn.close()
            else:
                # Check if we're in a quiz mode
                if serial_number in client_sessions and client_sessions[serial_number].get('current_quiz'):
                    session = client_sessions[serial_number]
                    ...
                    if serial_valid and "belajar menghitung" in esp_user.lower():
                        ...
                        if serial_number not in client_sessions:
                            client_sessions[serial_number] = {}
                        session = client_sessions[serial_number]
            ...
            return jsonify({'esp_user': esp_user, 
                           ...
                           'in_quiz': serial_number in client_sessions and client_sessions[serial_number].get('current_quiz') is not None, 
                           'quiz_type': client_sessions.get(serial_number, {}).get('current_quiz')}), 200
        except Exception as e:
